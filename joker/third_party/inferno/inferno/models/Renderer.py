...
class SRenderY(nn.Module):
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        transformed_vertices = torch.clone(transformed_vertices)
    ...
    def render_uv(self, transformed_vertices):
        '''
        -- rendering normal
        '''
        batch_size = transformed_vertices.shape[0]
    ...
    def render_uvinv(self, trans_verts):
        trans_verts = torch.clone(trans_verts)
        trans_verts[..., -1] -= trans_verts[..., -1].min()
        trans_verts[..., -1] /= trans_verts[..., -1].max()
        trans_verts[..., -1] *= 99
        trans_verts[..., -1] += 1

        depth_images = self.render_depth(trans_verts)
        uv_inv_images = self.world2uv(trans_verts)

        # visibility check
        visibility = torch.all((uv_inv_images[:, :2] >= -1) & (uv_inv_images[:, :2] <= 1), dim=1)
        sample_coords = uv_inv_images[:, :2].permute(0, 2, 3, 1)
        uv_depth_reprojected = torch.nn.functional.grid_sample(depth_images, sample_coords, mode="nearest",
                                                               padding_mode="zeros", align_corners=False)
        depth_match = torch.abs(uv_depth_reprojected[:, -1] - uv_inv_images[:, -1]) < 1
        visibility = visibility & depth_match
        uv_inv_images.permute(0, 2, 3, 1)[~visibility] = 0
        uv_inv_images[:, 2] = visibility.to(uv_inv_images.dtype)
        return uv_inv_images

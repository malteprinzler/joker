...
def revert_img_space_crop_to_orth_cam(cam, tform, res_1, res_2):
    """
    reverts image space cropping defined by tform (matrix of shape N x 3 x 3 converting coordinates from uncropped to cropped coordinate system)
    in parameters of orthographic projection camera (cam N x 3 with parameters s, tx, ty)
    res_1: image resolution before crop
    res_2: image resolution after crop

    returns new camera parameters of shape N x 3
    """
    s_cam = cam[:, :1]
    t_cam = cam[:, 1:]
    invert_y = np.ones_like(t_cam)
    invert_y[:, 1] = -1
    s_tform = np.mean(np.stack([tform[:, 0, 0], tform[:, 1, 1]], axis=-1), axis=-1, keepdims=True)
    assert np.all(np.std(np.stack([tform[:, 0, 0], tform[:, 1, 1]], axis=-1), axis=-1) / s_tform < 0.001)
    t_tform = tform[:, :2, 2]

    s_cam_out = s_cam / s_tform * res_2 / res_1
    t_cam_out = t_cam - invert_y * (t_tform / s_tform + (res_1 - 1) / 2 - (res_2 - 1) / 2 / s_tform) * 2 / s_cam_out / res_1

    return np.concatenate([s_cam_out, t_cam_out], axis=-1)


def revert_img_space_crop_to_persp_cam(intrinsics, tform):
    """
        reverts image space cropping defined by tform (matrix of shape N x 3 x 3 converting coordinates from uncropped to cropped coordinate system)
        in parameters of camera intrinsics ( N x 3 x 3 )

        returns new camera intrinsics of shape N x 3 x 3
        """
    tform_inv = np.linalg.inv(tform)
    intrinsics_new = tform_inv @ intrinsics
...
class FlameRenderer(Renderer):
    ...
    def forward(self, sample):
        ...
        # rendering into original image
        cam = cam.detach().cpu()
        H_orig = sample["image_original"].shape[-1]
        H = sample["image"].shape[-1]
        tform = sample["tform"]
        if isinstance(tform, torch.Tensor):
            tform = tform.detach().cpu().numpy()
        cam = revert_img_space_crop_to_orth_cam(cam, tform, res_1=H_orig, res_2=H)
        cam = torch.from_numpy(cam).to(verts.device).to(verts.dtype)
        sample["cam_out"] = cam
        sample["H_orig"] = [H_orig] * cam.shape[0]
        sample["H_crop"] = [H] * cam.shape[0]
        self.render.rasterizer.raster_settings.image_size = H_orig
...
class PerspectiveFlameRenderer(FlameRenderer):
    ...
    def forward(self, sample):
        ...
        # rendering into original image
        extrinsics = sample["extrinsics"].to(verts.device)
        intrinsics = sample["intrinsics"].to(verts.device)
        H_orig = sample["image_original"].shape[-1]
        H = sample["image"].shape[-1]
        tform = sample["tform"]
        if isinstance(tform, torch.Tensor):
        ...
        intrinsics = revert_img_space_crop_to_persp_cam(intrinsics, tform)
        intrinsics = torch.from_numpy(intrinsics).to(verts.device).to(verts.dtype)
        sample["H_orig"] = [H_orig] * extrinsics.shape[0]
        sample["H_crop"] = [H] * extrinsics.shape[0]
        self.render.rasterizer.raster_settings.image_size = H_orig

        # convert camera from screen into ndc space
        SCREEN_2_NDC = torch.tensor([[2 / H_orig, 0, -1], [0, 2 / H_orig, -1], [0, 0, 1.]], device=intrinsics.device, dtype=intrinsics.dtype)
        intrinsics_ndc = SCREEN_2_NDC @ intrinsics

        # world to camera
        cam = intrinsics_ndc @ extrinsics[:, :3]
        trans_verts = util.batch_persp_proj(verts, cam)

        if self.project_landmarks:
            if landmarks2d is not None:
                predicted_landmarks = util.batch_persp_proj(landmarks2d, cam)[:, :, :2]
            if landmarks3d is not None:
                predicted_landmarks_3d = util.batch_persp_proj(landmarks3d, cam)[:, :, :2]
            if landmarks2d_mediapipe is not None:
                predicted_landmarks_mediapipe = util.batch_persp_proj(landmarks2d_mediapipe, cam)[:, :, :2]
        ...
        sample["intrinsics_ndc"] = intrinsics_ndc
        return sample

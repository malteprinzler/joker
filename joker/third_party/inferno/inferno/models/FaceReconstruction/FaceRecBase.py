...
def renderer_from_cfg(cfg):
    ...
    elif cfg_renderer.type == "PerspectiveDecaRenderer":
        from inferno.models.temporal.Renderers import PerspectiveFlameRenderer
        renderer = PerspectiveFlameRenderer(cfg_renderer)
    else:
...
class FaceReconstructionBase(LightningModule):
    ...
    def visualize_batch(self, batch, batch_idx, prefix, in_batch_idx=None):
        ...
        normals = util.vertex_normals(verts, self.renderer.render.faces.expand(B, -1, -1))
        normal_images = self.renderer.render.render_normal(trans_verts, normals)
        uv_images = self.renderer.render.render_uv(trans_verts)
        uvinv_images = self.renderer.render.render_uvinv(trans_verts)
        ...
        for b in in_batch_idx:
            ...
            normal_image_np = np.clip(np.round(_torch_image2np(normal_images[b]) * 127.5 + 127.5), a_min=0, a_max=255).astype(np.uint8)
            normal_image_np[torch.all(normal_images[b] == 0, dim=0).cpu().numpy()] = 0
            visdict['normal_image'] += [normal_image_np]

            uv_image_np = np.clip(np.round(_torch_image2np(uv_images[b]) * 127.5 + 127.5), a_min=0, a_max=255).astype(np.uint8)
            uv_image_np[torch.all(uv_images[b] == 0, dim=0).cpu().numpy()] = 0
            visdict['uv_image'] += [uv_image_np]

            uvinv_image_np = np.clip(np.round(_torch_image2np(uvinv_images[b]) * 127.5 + 127.5), a_min=0, a_max=255).astype(np.uint8)
            uvinv_image_np[torch.all(uvinv_images[b] == 0, dim=0).cpu().numpy()] = 0
            visdict['uvinv_image'] += [uvinv_image_np]

...
from util.nvdiffrast import MeshRenderer, ndc_projection_from_bbx
...
class FaceReconModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.bbx_orig = input["bbx_orig"] if "bbx_orig" in input else None
        self.hw_orig = input["hw_orig"] if "hw_orig" in input else None
        self.img_orig = input["imgs_orig"].to(self.device) if "imgs_orig" in input else None

    def render_bfm_normals(self, coeff, persc_proj, rasterize_size, ndc_proj):
        coeff = self.facemodel.unsplit_coeff(coeff, dict(device=self.device))

        # storing default camera parameters
        facemodel_persc_proj_orig = copy.deepcopy(self.facemodel.persc_proj)
        ndc_proj_orig = copy.deepcopy(self.renderer.ndc_proj)
        h, w = copy.deepcopy(self.renderer.rasterize_size)

        # setting new camera parameters
        self.facemodel.persc_proj = torch.tensor(persc_proj, device=self.device)
        self.renderer.rasterize_size = rasterize_size
        self.renderer.ndc_proj = torch.tensor(ndc_proj, device=self.device)

        self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm, self.face_norm_roted = \
            self.facemodel.compute_for_render(coeff[None])
        self.pred_mask, _, self.pred_face = self.renderer(
            self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color)
        _, _, self.pred_normal = self.renderer(self.pred_vertex, self.facemodel.face_buf, feat=self.face_norm_roted)

        # restoring original camera parameters
        self.facemodel.persc_proj = facemodel_persc_proj_orig
        self.renderer.rasterize_size = (h, w)
        self.renderer.ndc_proj = ndc_proj_orig

        return ((self.pred_normal[0] * 127.5 + 127.5) * self.pred_mask[0]).round().clip(0, 255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)  # H x W x 3, 0...255

    def forward(self):
        output_coeff = self.net_recon(self.input_img)
        self.facemodel.to(self.device)

        if self.bbx_orig is not None:
            # changing camera parameters such that images are rendered in original frame
            h, w = self.input_img.shape[-2:]
            h_orig, w_orig = self.hw_orig
            h_crop = self.bbx_orig[3] - self.bbx_orig[1]
            w_crop = self.bbx_orig[2] - self.bbx_orig[0]
            facemodel_persc_proj_orig = copy.deepcopy(self.facemodel.persc_proj)
            new_proj = copy.deepcopy(self.facemodel.persc_proj)
            new_proj[:, 0] *= w_crop / w  # watch out: projection is transposed of actual projection matrix
            new_proj[:, 1] *= h_crop / h
            new_proj[-1, 0] += self.bbx_orig[0]
            new_proj[-1, 1] += h_orig - self.bbx_orig[3]  # watch out: coordinate systems origin is at bottom-left!
            self.facemodel.persc_proj = new_proj

            # undoing cropping for nvdiffrast in opengl like ndc space
            # note that
            # x' = bbx_left + (x'' + center)(2*center)*(bbx_width)
            # y' = h_orig - bbx_bottom + (y'' + center)*(bbx_height)/(2*center)
            # x' = coordinates in image space, coordinate origin bottom left
            # x''= cooridnates in ndc space normalized to -center ... +center with origin in center
            ndc_proj_orig = copy.deepcopy(self.renderer.ndc_proj)
            self.renderer.rasterize_size = (h_orig, w_orig)
            center = self.opt.center
            l = (0 - self.bbx_orig[0]) * (2 * center) / (self.bbx_orig[2] - self.bbx_orig[0]) - center
            r = (w_orig - self.bbx_orig[0]) * (2 * center) / (self.bbx_orig[2] - self.bbx_orig[0]) - center
            b = (h_orig - h_orig + self.bbx_orig[3]) * (2 * center) / (self.bbx_orig[3] - self.bbx_orig[1]) - center
            t = (0 - h_orig + self.bbx_orig[3]) * (2 * center) / (self.bbx_orig[3] - self.bbx_orig[1]) - center

            new_ndc_proj = torch.tensor(ndc_projection_from_bbx(self.opt.z_near, self.opt.z_far, self.opt.focal, l, t, r, b)).matmul(
                torch.diag(torch.tensor([1., -1, -1, 1])))
            self.renderer.ndc_proj = new_ndc_proj
        ...
        self.pred_coeffs_dict["facemodel_perc_proj"] = self.facemodel.persc_proj
        self.pred_coeffs_dict["rasterize_size"] = torch.tensor(self.renderer.rasterize_size)
        self.pred_coeffs_dict["ndc_proj"] = self.renderer.ndc_proj
    ...
    def compute_visuals(self):
        with torch.no_grad():
            base_img = (self.img_orig if self.img_orig is not None else self.input_img)
            input_img_numpy = 255. * base_img.detach().cpu().permute(0, 2, 3, 1).numpy()
            output_vis = self.pred_face * self.pred_mask + (1 - self.pred_mask) * base_img
            output_vis_normal = (self.pred_normal * .5 + .5) * self.pred_mask + (1 - self.pred_mask) * base_img
            output_vis_numpy_raw = 255. * output_vis.detach().cpu().permute(0, 2, 3, 1).numpy()
            output_vis_normal_raw = 255. * output_vis_normal.detach().cpu().permute(0, 2, 3, 1).numpy()
            ...
            output_vis_numpy = np.concatenate([output_vis_numpy, output_vis_normal_raw], axis=-2)

...
class SaverMixin:
    ...
    def create_loggers(self, cfg_loggers: DictConfig) -> None:
        if "wandb" in cfg_loggers.keys() and cfg_loggers.wandb.enable:
            self._wandb_logger = WandbLogger(
                project=cfg_loggers.wandb.project, name=cfg_loggers.wandb.name, save_dir=str(Path(self.get_save_dir()).parent), group=cfg_loggers.wandb.get("group", None)
            )
    ...
    def save_img_sequence(
        ...
        if forward_backward:
            imgs = imgs + imgs[-1: 0:-1]
            if start_from_center:
                quarter_length = len(imgs) // 4
                imgs = imgs[quarter_length:] + imgs[:quarter_length]
        ...
        if save_format == "gif":
            ...
            imgs = [cv2.cvtColor(i, cv2.COLOR_BGR2RGB) for i in imgs]
            imageio.mimsave(save_path, imgs, fps=fps, quality=10)
            assert ".mp4" in save_path
            # os.system(f"ffmpeg -i {save_path} -q:a 2 -q:v 2 -vcodec wmv2 -acodec wmav2 {save_path.replace('.mp4', '.avi')} -y")  # windows-compatible video format
        if name and self._wandb_logger:
            try:
                wandb.log(
                    {
                        name: wandb.Video(save_path, format="mp4", caption=caption),
                        "trainer/global_step": step,
                    }
                )
            except wandb.Error:
                warnings.warn("Wandb logging was not successful. Maybe wandb was not initialized?")
        return save_path
